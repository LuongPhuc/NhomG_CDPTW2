		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( isset( $args->has_children ) && $args->has_children && 0 === $depth && $args->depth > 1 ) {
				...
				$atts['id']            = 'menu-item-dropdown-' . $item->ID;
					...
					// '?type="'.$item->object_id.'"'
					if( $item->title == "HOME" || $item->title == "Home" || $item->title == "Trang ch?" &&  $depth === 0)
					{
							$atts['href'] = ! empty( $item->url ) ? $item->url : '#';
							$atts[' data-id'] = "";
					}
					// $atts['href'] = '#';
					$atts[' data-replace'] = $item->url;
				...
				if ( $depth > 0 ) {
					...
					// $atts['href'] = $item->url.'?type="'.$item->object_id.'"';
					// $atts[' data-url'] = $item->url.'?type='.$item->object_id;
